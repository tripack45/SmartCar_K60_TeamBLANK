...
u8 IsLinear(u8 boundaryX[BOUNDARY_NUM_MAX], u8 boundaryY[BOUNDARY_NUM_MAX], const u8 boundaryNum, float *alpha, float *beta, float *radius)
{
	/* Loop variable.*/
	u8 i = 0;
	...
	{
		abscissa[coordinateNum] = x = boundaryX[i];
		ordinate[coordinateNum] = y = boundaryY[i];
		sx += x;
		sy += y;
		x2 = x * x;
		sx2 += x2;
	...
	*beta = (float) ((s32) (sx2 * sy) - (s32) (sx * sxy) ) / denom;
	...
	ITM_EVENT8_WITH_PC(1,coordinateNum);
