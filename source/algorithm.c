...
s16 Speed_PID(u8 Expect){
  ...
  Error=Expect-Speed;
  Power=insert_in(Expect*EXP_SEN+MOTOR_PID_I*sumError/100+MOTOR_PID_P*Error +MOTOR_PID_D*(Error-motor_pid.LastError),-SPEED_MAX/2,SPEED_MAX);
  motor_pid.LastError=Error;
  sumError+=Error;
  if (tacho0) 
    return MOTOR_DEAD_RUN+100*Power/SPEED_MAX*MOTOR_PID_SENSITIVITY;
    //return 0 + 100*Power/SPEED_MAX*MOTOR_PID_SENSITIVITY; 
  else 
