...
//        ARM_MATH_CM4
//    List file    =  E:\freescale_racing\framework2016\FLASH\List\HMI.s
//
///////////////////////////////////////////////////////////////////////////////
...
LED2:
        ...
        BEQ.N    ??LED2_0
//   43     PTA->PSOR |= 1<<7;
        LDR.N    R0,??DataTable7_1  ;; 0x400ff004
        LDR      R0,[R0, #+0]
        ORRS     R0,R0,#0x80
        LDR.N    R1,??DataTable7_1  ;; 0x400ff004
        STR      R0,[R1, #+0]
        B.N      ??LED2_1
//   44   else
//   45     PTA->PCOR |= 1<<7;
??LED2_0:
        LDR.N    R0,??DataTable7_2  ;; 0x400ff008
        LDR      R0,[R0, #+0]
        ORRS     R0,R0,#0x80
        LDR.N    R1,??DataTable7_2  ;; 0x400ff008
        STR      R0,[R1, #+0]
//   46 }
??LED2_1:
        BX       LR               ;; return
...
//   48   PTA->PTOR |= 1<<7;
LED2_Tog:
        LDR.N    R0,??DataTable7_3  ;; 0x400ff00c
        LDR      R0,[R0, #+0]
        ORRS     R0,R0,#0x80
        LDR.N    R1,??DataTable7_3  ;; 0x400ff00c
        STR      R0,[R1, #+0]
//   49 }
        BX       LR               ;; return
...
Battery:
        MOVS     R0,#+0
        LDR.N    R1,??DataTable7_4  ;; 0x400bb000
        STR      R0,[R1, #+0]
//   74   while((ADC1->SC1[0]&ADC_SC1_COCO_MASK)==0);
??Battery_0:
        LDR.N    R0,??DataTable7_4  ;; 0x400bb000
        LDR      R0,[R0, #+0]
        LSLS     R0,R0,#+24
        BPL.N    ??Battery_0
//   75   tmp = ADC1->R[0];
        LDR.N    R0,??DataTable7_5  ;; 0x400bb010
        LDR      R0,[R0, #+0]
//   76   //
//   77   return tmp;
...
HMI_Init:
        LDR.N    R0,??DataTable7_6  ;; 0x40049018
        LDR      R0,[R0, #+0]
        ORRS     R0,R0,#0x100
        LDR.N    R1,??DataTable7_6  ;; 0x40049018
        STR      R0,[R1, #+0]
//   87   PORTA->PCR[7] |= PORT_PCR_MUX(1);
        LDR.N    R0,??DataTable7_7  ;; 0x4004901c
        LDR      R0,[R0, #+0]
        ORRS     R0,R0,#0x100
        LDR.N    R1,??DataTable7_7  ;; 0x4004901c
        STR      R0,[R1, #+0]
//   88   PTA->PDDR |= (3<<6);
        LDR.N    R0,??DataTable7_8  ;; 0x400ff014
        LDR      R0,[R0, #+0]
        ORRS     R0,R0,#0xC0
        LDR.N    R1,??DataTable7_8  ;; 0x400ff014
        STR      R0,[R1, #+0]
//   89   PTA->PDOR |= (3<<6);
        LDR.N    R0,??DataTable7_9  ;; 0x400ff000
        LDR      R0,[R0, #+0]
        ORRS     R0,R0,#0xC0
        LDR.N    R1,??DataTable7_9  ;; 0x400ff000
        STR      R0,[R1, #+0]
//   90 
//   91   //===== KEY =====
//   92   PORTA->PCR[8] |= PORT_PCR_MUX(1);
        LDR.N    R0,??DataTable7_10  ;; 0x40049020
        LDR      R0,[R0, #+0]
        ORRS     R0,R0,#0x100
        LDR.N    R1,??DataTable7_10  ;; 0x40049020
        STR      R0,[R1, #+0]
//   93   PORTA->PCR[9] |= PORT_PCR_MUX(1);
        LDR.N    R0,??DataTable7_11  ;; 0x40049024
        LDR      R0,[R0, #+0]
        ORRS     R0,R0,#0x100
        LDR.N    R1,??DataTable7_11  ;; 0x40049024
        STR      R0,[R1, #+0]
//   94   PORTA->PCR[10] |= PORT_PCR_MUX(1);
        LDR.N    R0,??DataTable7_12  ;; 0x40049028
        LDR      R0,[R0, #+0]
        ORRS     R0,R0,#0x100
        LDR.N    R1,??DataTable7_12  ;; 0x40049028
        STR      R0,[R1, #+0]
//   95   PTA->PDDR &=~(7<<8);
        LDR.N    R0,??DataTable7_8  ;; 0x400ff014
        LDR      R0,[R0, #+0]
...
//  101   PORTC->PCR[4] |= PORT_PCR_MUX(1);
        LDR.N    R0,??DataTable7_13  ;; 0x4004b010
        LDR      R0,[R0, #+0]
        ORRS     R0,R0,#0x100
        LDR.N    R1,??DataTable7_13  ;; 0x4004b010
        STR      R0,[R1, #+0]
//  102   PORTC->PCR[5] |= PORT_PCR_MUX(1);
        LDR.N    R0,??DataTable7_14  ;; 0x4004b014
        LDR      R0,[R0, #+0]
        ORRS     R0,R0,#0x100
        LDR.N    R1,??DataTable7_14  ;; 0x4004b014
        STR      R0,[R1, #+0]
//  103   PORTC->PCR[6] |= PORT_PCR_MUX(1);
        LDR.N    R0,??DataTable7_15  ;; 0x4004b018
        LDR      R0,[R0, #+0]
        ORRS     R0,R0,#0x100
        LDR.N    R1,??DataTable7_15  ;; 0x4004b018
        STR      R0,[R1, #+0]
//  104   PORTC->PCR[7] |= PORT_PCR_MUX(1);
        LDR.N    R0,??DataTable7_16  ;; 0x4004b01c
        LDR      R0,[R0, #+0]
        ORRS     R0,R0,#0x100
        LDR.N    R1,??DataTable7_16  ;; 0x4004b01c
        STR      R0,[R1, #+0]
//  105   PTC->PDDR &=~(0xf<<4);
        LDR.N    R0,??DataTable7_17  ;; 0x400ff094
        LDR      R0,[R0, #+0]
        BICS     R0,R0,#0xF0
        LDR.N    R1,??DataTable7_17  ;; 0x400ff094
        STR      R0,[R1, #+0]
...
//  112   PORTD->PCR[15] |= PORT_PCR_MUX(1);
        LDR.N    R0,??DataTable7_18  ;; 0x4004c03c
        LDR      R0,[R0, #+0]
        ORRS     R0,R0,#0x100
        LDR.N    R1,??DataTable7_18  ;; 0x4004c03c
        STR      R0,[R1, #+0]
//  113   PTD->PDDR |= (1<<15);
        LDR.N    R0,??DataTable7_19  ;; 0x400ff0d4
        LDR      R0,[R0, #+0]
        ORRS     R0,R0,#0x8000
        LDR.N    R1,??DataTable7_19  ;; 0x400ff0d4
        STR      R0,[R1, #+0]
//  114   PTD->PDOR &=~ (1<<15);
        LDR.N    R0,??DataTable7_20  ;; 0x400ff0c0
        LDR      R0,[R0, #+0]
        BICS     R0,R0,#0x8000
        LDR.N    R1,??DataTable7_20  ;; 0x400ff0c0
        STR      R0,[R1, #+0]
//  115 
//  116   //==== battery ====
//  117   
...
//  119     SIM->SCGC3 |= SIM_SCGC3_ADC1_MASK;  //ADC1 Clock Enable
        LDR.N    R0,??DataTable7_22  ;; 0x40048030
        LDR      R0,[R0, #+0]
        ORRS     R0,R0,#0x8000000
        LDR.N    R1,??DataTable7_22  ;; 0x40048030
        STR      R0,[R1, #+0]
//  120     ADC1->CFG1 |= 0
//  121                //|ADC_CFG1_ADLPC_MASK
//  122                | ADC_CFG1_ADICLK(1)
//  123                | ADC_CFG1_MODE(1);
        LDR.N    R0,??DataTable7_23  ;; 0x400bb008
        LDR      R0,[R0, #+0]
        ORRS     R0,R0,#0x5
        LDR.N    R1,??DataTable7_23  ;; 0x400bb008
        STR      R0,[R1, #+0]
//  124                //| ADC_CFG1_ADIV(0);
//  125     ADC1->CFG2 |= //ADC_CFG2_ADHSC_MASK |
//  126                   ADC_CFG2_ADACKEN_MASK; 
        LDR.N    R0,??DataTable7_24  ;; 0x400bb00c
        LDR      R0,[R0, #+0]
        ORRS     R0,R0,#0x8
        LDR.N    R1,??DataTable7_24  ;; 0x400bb00c
        STR      R0,[R1, #+0]
//  127     
